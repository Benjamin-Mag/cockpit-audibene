...
// aucun appel réseau, chargé seulement quand un onglet en a besoin.
...
export const kmLabel = (km: number) => (km < 1 ? '< 1 km' : km < 10 ? `${km.toFixed(1).replace('.', ',')} km` : `${Math.round(km)} km`);
